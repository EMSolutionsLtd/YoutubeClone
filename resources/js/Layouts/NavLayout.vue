...
const isNavOverlay = () => {
    ...
    if(usePage().url !== '/') openSideNavOverlay.value = !openSideNavOverlay.value
}

let searchQuery = ref('')
const doSearch = () => {
    if(searchQuery.value) {
        router.get(`/search?keyword=${searchQuery.value}`);
    }
}
...
<template>
    <div class="relative">
            ...
            <div class="w-[600px] md:block hidden">
                <div class="rounded-full flex items-center bg-[#222222] ">
                    <input
                        ...
                        placeholder="Search"
                    />
                    <MagnifyIcon class="mx-6 hover:cursor-pointer" fillColor="#FFFFFF" :size="23" @click="doSearch"/>
                </div>
            </div>
            <div>
                <img
        ...
        <div
            ...
            :class="{
                'w-[calc(100%-70px)]': !openSideNav && $page.url == '/' && width > 670,
                'w-[calc(100%-240px)]': openSideNav && $page.url == '/' && width > 670,
                'xl:w-[calc(100%-80px)]': $page.url !== '/',
                // 'w-[100vw]': width < 639
            }"
        >
            <slot />
